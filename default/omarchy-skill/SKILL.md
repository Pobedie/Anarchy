...
description: >
  ...
  workspace settings, display config, and user-facing omarchy commands. Excludes ANARCHY
  source development in ~/.local/share/omarchy/ and `omarchy dev` workflows.
---

# ANARCHY Skill

Manage [ANARCHY](https://omarchy.org/) Linux systems - a beautiful, modern, opinionated Arch Linux distribution with Hyprland.
...
**Do NOT use this skill for ANARCHY development tasks** (editing files in `~/.local/share/omarchy/`, creating migrations, or running `omarchy dev ...` workflows).
...
ANARCHY ships a single `omarchy` CLI that dispatches to all `omarchy-*` binaries via `omarchy <group> <action>`. Always prefer this form — it is self-documenting and stable. The underlying `omarchy-*` binaries still exist on `PATH` and remain safe to read for source.
...
omarchy version                 # Show ANARCHY version
