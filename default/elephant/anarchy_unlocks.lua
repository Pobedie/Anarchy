--
-- Dynamic ANARCHY Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the anarchy-shipped Plymouth via
-- anarchy-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs anarchy-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "anarchyunlocks"
NamePretty = "ANARCHY Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/anarchy/themes"
  local anarchy_path = os.getenv("ANARCHY_PATH") or ""
  local default_themes_dir = anarchy_path .. "/themes"
  local default_preview = anarchy_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "anarchy-launch-floating-terminal-with-presentation "
                  .. shell_escape("anarchy-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "anarchy-launch-floating-terminal-with-presentation "
          .. shell_escape("anarchy-plymouth-reset"),
    },
  }
