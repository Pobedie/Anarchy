...
local function command_from(value, description)
  ...
  if value.anarchy then
    return "anarchy-launch-" .. value.anarchy
  ...
  elseif value.tui then
    if value.focus then
      return "anarchy-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "anarchy-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "anarchy-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "anarchy-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "anarchy-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_menu(keys, description, menu, options)
  o.bind(keys, description, menu and ("anarchy-menu " .. menu) or "anarchy-menu", options)
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "anarchy-toggle-" .. toggle, options)
end
