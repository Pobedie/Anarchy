if command -v limine &>/dev/null; then
  ...
  sudo tee /etc/mkinitcp_hooks.conf <<EOF >/dev/null
HOOKS=(base udev plymouth keyboard autodetect microcode modconf kms keymap consolefont block $ENCRYPTION_HOOK filesystems fsck btrfs-overlayfs)
EOF
...
EOF
  ...
  sudo cp $ANARCHY_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $ANARCHY_PATH/default/limine/limine.conf /boot/limine.conf
  ...
  sudo cp $ANARCHY_PATH/default/snapper/root /etc/snapper/configs/root
